...
export const SEMANTIC_RULESET_VERSION = '1.0.0';
export const GEOMETRY_CROSSCHECK_VERSION = '1.0.0';
...
export async function executeSemanticValidation(input: { fusion: FusionStageResult; now?: string }): Promise<SemanticValidationExecutionResult> {
  ...
  const add = (ruleId: string, severity: ValidationSeverity, entityKeys: string[], fieldNames: string[], candidateIds: string[], message: string) => {
    issues.push({
      ...
      ruleId,
      ruleVersion: ruleId.startsWith('GEOMETRY_') ? GEOMETRY_CROSSCHECK_VERSION : SEMANTIC_RULESET_VERSION,
      severity,
      status: 'OPEN',
  ...
  if (airportIcao !== undefined && !/^[A-Z]{4}$/.test(String(airportIcao))) {
  ...
  for (const runway of input.fusion.entities.filter((item) => item.entityType === 'RUNWAY').flatMap((item) => values(item.fields.runway).map((value) => [item, value] as const))) {
    if (!/^(?:RW)?(?:0[1-9]|[12][0-9]|3[0-6])[LCR]?$/.test(String(runway[1]).toUpperCase())) {
  ...
  const seenSequence = new Map<number, CanonicalEntity>();
  ...
  for (const leg of legs) {
    ...
    else if (seenSequence.has(sequence)) add('LEG_SEQUENCE_UNIQUE', 'BLOCKING', [seenSequence.get(sequence)!.entityKey, leg.entityKey], ['sequence'], [...candidateIds(seenSequence.get(sequence)!, 'sequence'), ...candidateIds(leg, 'sequence')], `航段序号 ${sequence} 重复。`);
    else seenSequence.set(sequence, leg);
  ...
  const output: ValidationStageResult = {
    ...
    blockingIssueCount,
    reviewIssueCount,
    ruleVersions: { semantic: SEMANTIC_RULESET_VERSION, geometryCrosscheck: GEOMETRY_CROSSCHECK_VERSION },
  ...
  for (const leg of legs) {
    ...
    if (fromFix && toFix) {
      ...
      if (start && end) {
        ...
        if (publishedCourse !== undefined && angularDifference(publishedCourse, calculatedCourse) > 20) {
