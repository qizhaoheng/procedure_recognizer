...
- DME conditions from a navaid, such as `0.5 DME <VOR>`, `2.6 DME <VOR>`, `13D <VOR>`, where
  `<VOR>` is the VOR/DME ident printed on the current chart
- VOR/DME radials and cross-radials, such as `RDL 195 <VOR>`, `RDL-114 <VOR>`, and radial intercepts
...
- Do not collapse a departure into `CA + final named fix` when the chart depicts or the 424/table implies
  ...
  final `CF` leg.
- Read the black solid path and arrowheads as the flown procedure track. Read dashed `RDLxxx <VOR>` lines
  as VOR/DME radial references or intercept conditions, not as airway names and not as named fixes.
- Procedure-name labels printed along a line (fix name + designator, e.g. `<FIX> 1L`) are
  procedure-track labels. Do not attach them to a fix point. The named fixes are the triangle fix
  symbols themselves.
...
- `CF` is the final course-to-fix/radial leg. Use fixIdentifier as the named transition fix,
  recommendedNavaid = the VOR/DME ident when the fix is defined by that navaid's radial/DME, and
  end this procedure there.
...
leg decomposition and label completeness; never copy its values — read every course, distance,
altitude, and ident from the current chart and table.
...
- Use AD 2.19 navaid summaries when sent; a VOR/DME ident used in the chart's DME or radial text is a usedInProcedure navaid.
